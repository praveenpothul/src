    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{overflow:'hidden'}}>
        <CarouselContainer ref={carouselRef} items={items}>
          {items.map((item) => (
            <CardContainer key={item.id} data-title={type=='Client'?item.name:item.category}>
              <CardImage src={imageRendererFunc(type=='Client'?item.preferredCategory:item.category)} alt={item.name} onClick={()=>
                handleNavigation(type,item.id )
               }/>
